MRuby::Toolchain.new(:arm) do |conf|
  [conf.cc, conf.objc, conf.asm].each do |cc|
    cc.command = ENV['CC'] || 'arm-linux-gnueabi-gcc'
  ...
  [conf.cxx].each do |cxx|
    cxx.command = ENV['CXX'] || 'arm-linux-gnueabi-g++'
  ...
  conf.linker do |linker|
    linker.command = ENV['LD'] || 'arm-linux-gnueabi-gcc'
  ...
  conf.archiver do |ar|
    ar.command = ENV['AR'] || 'arm-linux-gnueabi-ar'
  end
